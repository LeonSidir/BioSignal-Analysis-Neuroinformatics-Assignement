...
%% PART III: Identifying the Least Informative Sensor

% Reshape the Z scores to group them by sensor. Z is currently a 1x228 vector. We reshape it to 3x76 
Z_per_sensor = reshape(Z, 3, 76);

% Calculate a total score for each sensor
Sensor_Scores = sum(abs(Z_per_sensor), 1);

% Find the sensor with the LOWEST score
[min_score, worst_sensor_idx] = min(Sensor_Scores);

disp('------------------------------------------------');
disp(['The least informative sensor is Sensor No: ', num2str(worst_sensor_idx)]);

% Select the features corresponding ONLY to this worst sensor
start_idx = (worst_sensor_idx - 1) * 3 + 1;
feature_indices = start_idx : (start_idx + 2);

XX_worst = XX(feature_indices, :); % Extract only these 3 rows

% Classification Performance using ONLY the worst sensor
C_worst = classify(XX_worst', XX_worst', GROUP);
